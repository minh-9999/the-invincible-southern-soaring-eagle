...
#include "../chess/ChessBoardWidget.h"
#include "../chess/BackgroundWidget.h"
    ...
    :  QObject(), m_chessBoard(chessBoard), m_backgroundWidget(backgroundWidget)
{
    ...
    else
    {
        QMessageBox::warning(nullptr,
                             tr("Warning"),
                             tr("Warning: ClockWidget is null in MenuSettings!"));
    }
...
void MenuSettings::settingsMenus(QMainWindow* window, ChessBoardWidget* boardWidget, BackgroundWidget *backgroundWidget)
{
    ...
    // Reuse toggle actions from boardWidget (arrows, coordinates)
    ...
    // Reuse piece size submenu from boardWidget
    menuSettings->addMenu(boardWidget->pieceSizeMenu);

    // menuSettings->addSeparator();
    MenuUtils::addFullWidthSeparator(menuSettings, 2);

    // Reuse background toggle and change actions from BackgroundWidget
    ...
    // Reuse custom theme menu from ChessBoardWidget
    menuSettings->addMenu(boardWidget->customThemeMenu);

    boardWidget->PieceSetMenu();
    menuSettings->addMenu(boardWidget->m_pieceSetMenu);
...
void MenuSettings::onLoadCustomBoard()
{
    ...
    if (!fileName.isEmpty())
    {
        m_chessBoard->loadCustomBoard(fileName);
    }
}

void MenuSettings::onLoadCustomPieces()
{
    if (!m_chessBoard) return;
    ...
    {
        m_chessBoard->loadCustomPieces(folderPath);
    }
}

void MenuSettings::onResetCustomTheme()
{
    if (!m_chessBoard) return;
    m_chessBoard->resetCustomTheme();
}
